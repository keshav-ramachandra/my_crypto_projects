...
contract Controller{
    ...
    // virtualnodeId => user => renBalance
    mapping(uint => mapping(address => uint)) public renTokenBalances;
    //virtualnodeId => bonds
    mapping(uint => uint) public currentNodeBonds;
    
    uint public nodeCount;
    
    //nodeId => darkNodeId 
    mapping(uint => address) internal nodeMappings; 
    
    //darkNodeId => publicKeys
    mapping(address => bytes) private publicKeys;
    
    //darkNodeId => it's state
    mapping(address => bool) darknodeStates;
    
    //darkNodeId => registring request times
    mapping(address => uint256) nodesRegistrationTime;
    ...
    function setDarknodeId(uint _nodeId, address _darknodeId) onlyCreator() public returns(bool){
        ...
        require(nodeMappings[_nodeId] == address(0),"Node is already mapped to a darknodeId");
        nodeMappings[_nodeId] = _darknodeId;
        return true;
    }
    
    function setPublickKey(uint _nodeId, bytes memory _key) onlyCreator() public returns(bool){
        //check if node number is mapped to a real darknodeId
        require(nodeMappings[_nodeId] != address(0),"Node is not yet mapped to a darknodeId");
        publicKeys[nodeMappings[_nodeId]] = _key;
        return true;
    }
    ...
    //DarknodeRegistryStateV1 private darknodeRegState;
   ...
   // DarknodePaymentStore private darknodePaymentStoreV1;
   
    constructor (IERC20 _token,address _darknodeProxyAddress,address payable _darknodePaymentAddress) public {
        owner = msg.sender;
        token = _token;
        //used for retrieving the epoch time and other state variables like registryStore
        darknodeStateV1 = DarknodeRegistryStateV1(_darknodeProxyAddress);
        // Using to perform Registration
        darknodeLogicV1 = DarknodeRegistryLogicV1(_darknodeProxyAddress);
        // Using to distribute the rewards
        darknodePayment = DarknodePayment(_darknodePaymentAddress);
        //darknodePaymentStoreV1 = DarknodePaymentStore(_darknodePaymentAddress);
        
        nodeCount = nodeCount + 1;
        currentNodeBonds[nodeCount] = 0;
    }
    
    
    // Accept Ren deposits
    function stakeRen(uint _amount) public payable
    {
        require(msg.value == 0, "unexpected ether transfer");
        token.transferFrom(msg.sender, address(this), _amount);
        renTokenBalances[nodeCount][msg.sender] = _amount;
        currentNodeBonds[nodeCount] = currentNodeBonds[nodeCount] + _amount;
        if(isBondSufficient(nodeCount)){
            require(nodeMappings[nodeCount] != address(0),"Node does not have a public key");
            darknodeLogicV1.register(nodeMappings[nodeCount],publicKeys[nodeMappings[nodeCount]]);
            nodesRegistrationTime[nodeMappings[nodeCount]] = block.timestamp;
            nodeCount = nodeCount + 1;
            
        }
        //emit LogPaymentReceived(msg.sender, _amount, token);
    }
    
    event LogPaymentReceived(address,uint256,IERC20);
    
    
    // Get share for a particular node
    function getTokenShareByNode(uint _nodeId) public view returns(uint){
        return renTokenBalances[_nodeId][msg.sender];
    }
    
    // Get my total shares
    function getTotalTokenShare() public view returns(uint){
        uint total = 0;
        for (uint i = 1; i <= nodeCount; i++) {
            total = total + getTokenShareByNode(i);
        }
        return total;
    }
    
    function isBondSufficient(uint _nodeId) internal view returns(bool){
        if(currentNodeBonds[_nodeId] >= darknodeStateV1.minimumBond()){
            return true;
        }
        else{
            return false;
    ...
    function getRewards(address _darkNodeId, address _tokenAddress) public onlyCreator() {
        require(nodesRegistrationTime[_darkNodeId] > getEpochTime(),"Node is not yet registered as epoch time has not passed");
        require(nodesRegistrationTime[_darkNodeId] > getNextEpochTime(),"At least one Payment cycle must be passed to claim rewards");
        darknodePayment.withdraw(_darkNodeId,_tokenAddress);
    }
