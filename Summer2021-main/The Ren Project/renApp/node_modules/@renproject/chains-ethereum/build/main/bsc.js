"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.BinanceSmartChain = exports.BinanceSmartChainClass = exports.BscConfigMap = exports.renBscMainnet = exports.renBscDevnet = exports.renBscTestnet = void 0;
const interfaces_1 = require("@renproject/interfaces");
const utils_1 = require("@renproject/utils");
const ethereum_1 = require("./ethereum");
const networks_1 = require("./networks");
const utils_2 = require("./utils");
exports.renBscTestnet = {
    name: "BSC Testnet",
    chain: "bscTestnet",
    isTestnet: true,
    chainLabel: "BSC Testnet",
    networkID: 97,
    addresses: {
        GatewayRegistry: "0x838F881876f53a772D2F8E2f8aa2e4a996431495",
        BasicAdapter: "0x7de1253A8da6620351ec477b38BdC6a55FCd0f85",
    },
    publicProvider: () => `https://data-seed-prebsc-1-s1.binance.org:8545/`,
    explorer: networks_1.StandardExplorer("https://testnet.bscscan.com"),
    /** @deprecated Renamed to publicProvider. Will be removed in 3.0.0. */
    infura: "https://data-seed-prebsc-1-s1.binance.org:8545/",
    /** @deprecated Renamed to explorer. Will be removed in 3.0.0. */
    etherscan: "https://testnet.bscscan.com",
};
exports.renBscDevnet = Object.assign(Object.assign({}, exports.renBscTestnet), { addresses: {
        GatewayRegistry: "0x87e83f957a2F3A2E5Fe16d5C6B22e38FD28bdc06",
        BasicAdapter: "0x105435a9b0f375B179e5e43A16228C04F01Fb2ee",
    } });
exports.renBscMainnet = {
    name: "BSC Mainnet",
    chain: "bscMainnet",
    isTestnet: false,
    chainLabel: "BSC Mainnet",
    networkID: 56,
    addresses: {
        GatewayRegistry: "0x21C482f153D0317fe85C60bE1F7fa079019fcEbD",
        BasicAdapter: "0xAC23817f7E9Ec7EB6B7889BDd2b50e04a44470c5",
    },
    publicProvider: () => `https://bsc-dataseed.binance.org`,
    explorer: networks_1.StandardExplorer("https://bscscan.com"),
    /** @deprecated Renamed to publicProvider. Will be removed in 3.0.0. */
    infura: "https://bsc-dataseed.binance.org",
    /** @deprecated Renamed to explorer. Will be removed in 3.0.0. */
    etherscan: "https://bscscan.com",
};
exports.BscConfigMap = {
    [interfaces_1.RenNetwork.MainnetVDot3]: exports.renBscMainnet,
    [interfaces_1.RenNetwork.TestnetVDot3]: exports.renBscTestnet,
    [interfaces_1.RenNetwork.DevnetVDot3]: exports.renBscDevnet,
};
const resolveBSCNetwork = (renNetwork) => {
    if (!renNetwork) {
        return exports.BscConfigMap[interfaces_1.RenNetwork.MainnetVDot3];
    }
    if (renNetwork.addresses) {
        return renNetwork;
    }
    else {
        const details = interfaces_1.getRenNetworkDetails(
        // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
        renNetwork);
        return details.isTestnet
            ? details.name === interfaces_1.RenNetwork.DevnetVDot3
                ? exports.renBscDevnet
                : exports.renBscTestnet
            : exports.renBscMainnet;
    }
};
class BinanceSmartChainClass extends ethereum_1.EthereumClass {
    constructor(web3Provider, renNetwork) {
        // To be compatible with the Ethereum chain class, the first parameter
        // is a web3Provider and the second the RenVM network. However,
        super(web3Provider, resolveBSCNetwork(renNetwork));
        this.chain = BinanceSmartChainClass.chain;
        this.name = BinanceSmartChainClass.chain;
        this.legacyName = undefined;
        this.logRequestLimit = 5000;
        this.configMap = exports.BscConfigMap;
        this.utils = utils_1.utilsWithChainNetwork(BinanceSmartChainClass.utils, () => this.renNetworkDetails);
        this.initialize = (renNetwork) => {
            this.renNetworkDetails =
                this.renNetworkDetails ||
                    exports.BscConfigMap[interfaces_1.getRenNetworkDetails(renNetwork).name];
            if (!this.renNetworkDetails) {
                throw new Error(`Unable to set ${this.name} network for RenVM network ${interfaces_1.getRenNetworkDetails(renNetwork).name}. Please provide ${this.name} network details to ${this.name} constructor.`);
            }
            return this;
        };
    }
}
exports.BinanceSmartChainClass = BinanceSmartChainClass;
BinanceSmartChainClass.chain = "BinanceSmartChain";
BinanceSmartChainClass.configMap = exports.BscConfigMap;
BinanceSmartChainClass.utils = {
    resolveChainNetwork: resolveBSCNetwork,
    addressIsValid: utils_2.addressIsValid,
    transactionIsValid: utils_2.transactionIsValid,
    addressExplorerLink: (address, network) => `${(exports.BinanceSmartChain.utils.resolveChainNetwork(network) ||
        exports.renBscMainnet).etherscan}/address/${address}`,
    transactionExplorerLink: (transaction, network) => `${(exports.BinanceSmartChain.utils.resolveChainNetwork(network) ||
        exports.renBscMainnet).etherscan}/tx/${transaction || ""}`,
};
// @dev Removes any static fields, except `utils`.
exports.BinanceSmartChain = utils_1.Callable(BinanceSmartChainClass);
//# sourceMappingURL=bsc.js.map