...
class FilecoinClass {
    constructor(network, _a = {}) {
        ...
        this.getDeposits = async (asset, address, progress, onDeposit) => {
            ...
            if (this.filfox && (!progress || progress === 0)) {
                try {
                    ...
                    let page = 0;
                    while (true) {
                        const { deposits, totalCount } = await this.filfox.fetchDeposits(address.address, address.params, page, size);
            ...
            let txs;
            ({ txs, progress } = await lotus_1.fetchDeposits(this.client, address.address, address.params, this.chainNetwork, progress || 0));
        ...
        this.transactionConfidence = async (transaction) => {
            ...
            transaction = await lotus_1.fetchMessage(this.client, transaction.cid, this.chainNetwork);
            return {
        ...
        this.getGatewayAddress = (asset, compressedPublicKey, gHash) => {
            ...
            const address = filecoin_address_1.encode(networkPrefix, addressObject);
        ...
        this.transactionFromRPCFormat = async (txid, _txindex) => {
            ...
            return lotus_1.fetchMessage(this.client, typeof txid === "string" ? txid : new cids_1.default(txid).toString(), this.chainNetwork);
        };
        /**
