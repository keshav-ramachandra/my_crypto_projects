...
async function main() {
  ...
  const fullRequestClient = new ethers.Contract(
    util.readFromLogJson('FullRequestClient address'),
  ...
  console.log('',parameters.providerId);
  async function makeRequest() {
    const receipt = await fullRequestClient.makeRequest(
  ...
  console.log(`${coinId} price is ${(await fullRequestClient.fulfilledData(requestId)) / 1e6} USD`);
}
