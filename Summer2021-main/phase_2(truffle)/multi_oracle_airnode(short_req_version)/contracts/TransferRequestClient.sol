...
contract TransferRequestClient is AirnodeClient,Ownable{
    ...
    function exchangeEthForToken(bytes32 _template_id, string memory _token_name, address _exchange_address) public payable isUserRegistered(msg.sender) ethCheck(msg.value){

        bytes32 requestId = airnode.makeShortRequest(
            _template_id,
            abi.encode(
                bytes32("1S"),
                bytes32("coinId"),"ethereum"
                       )
        );
